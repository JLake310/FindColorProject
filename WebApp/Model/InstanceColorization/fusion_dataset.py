...
class Fusion_Testing_Dataset(Data.Dataset):
    def __init__(self, opt, box_num=8):
        self.PRED_BBOX_DIR = '{0}_bbox'.format(opt.test_img_dir) # pred box 경로
        self.IMAGE_DIR = opt.test_img_dir # image 경로
        ...
        self.box_num = opt.box_num ###

    def __getitem__(self, index):
        pred_info_path = join(self.PRED_BBOX_DIR, self.IMAGE_ID_LIST[index].split('.')[0] + '.npz')
        output_image_path = join(self.IMAGE_DIR, self.IMAGE_ID_LIST[index]) # 전체 full image에서 하나 고름

        # os.makedirs(output_image_path.split(".")[0]) ###
        ...
        print("="*20)
        print(f"number of boxes : {len(pred_bbox)}")
        ...
        ############ crop the car
        #     crop = pil_img.crop((startx, starty, endx, endy)) 
        #     crop.save(f"/home/saebyeol/colorization/InstColorization/cropped/detected_taxi/crop_{i}.png", "PNG")
        
        # cropped_img_list = []
        # for img_path in glob.glob("/home/saebyeol/colorization/InstColorization/cropped/colorized/*"):
        #     print(img_path)
        #     img = read_to_pil(img_path)
