...
def gen_maskrcnn_bbox_fromPred(pred_data_path, box_num_upbound=-1):
    ...
    pred_bbox = pred_data['bbox'].astype(np.int32) # pred box를 뽑고

    # if box_num_upbound > 0 and pred_bbox.shape[0] > box_num_upbound: # detect된 bbox가 8개(bbox최대개수 정해놓은거)보다 많으면
    #     pred_scores = pred_data['scores']
    #     index_mask = np.argsort(pred_scores, axis=0)[pred_scores.shape[0] - box_num_upbound: pred_scores.shape[0]] # argsort하고 indexing (confidence낮은걸 버림)
